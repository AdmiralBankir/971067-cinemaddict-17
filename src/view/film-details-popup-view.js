...
import { getHumanizeDurationFromMinutes, getHumanizeDate, getTimeFromDate} from '../utils.js';
...
const createFilmDetailsPopupTemplate = (film, comments) => {
  ...
  const releaseDate = getHumanizeDate(release.date);
  const humanizeDuration = getHumanizeDurationFromMinutes(runtime);

  const genresTemplate = genre.map((g) => `<span class="film-details__genre">${g}</span>`).join('');

  const generateCommentItem = (commentItem) => {
    ...
    <div>
      ...
      <p class="film-details__comment-info">
        ...
        <span class="film-details__comment-day">${getTimeFromDate(date)}</span>
  ...
  const generateCommentList = () => (`
    <ul class="film-details__comments-list">
      ${comments.map((item) => generateCommentItem(item)).join('')}
    </ul>
  `);

  const commentsListTepmplate = comments.length ? generateCommentList() : '';
  return(
    `
